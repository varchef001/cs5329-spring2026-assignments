...
def generate_adversarial(n: int, seed: int) -> Dict[str, Any]:
    ...
    tasks.append(make_task(0, 0, max(10, n), 60.0 + n * 0.5, 5, "compute"))

    current = 0
    for task_id in range(1, n):
        duration = 2 if task_id % 2 == 1 else 3
        start = current
        end = start + duration
        current = end
        weight = rng.uniform(7.0, 10.0)
        tasks.append(make_task(task_id, start, end, weight, 5, "io" if task_id % 3 else "network"))
...
def validation_cases() -> Dict[str, Dict[str, Any]]:
    return {
        ...
        "validation_identical_6.json": {
            ...
            "category_limit": 2,
            "tasks": [
                make_task(0, 0, 4, 10.0, 2, "compute"),
                make_task(1, 0, 4, 10.0, 2, "compute"),
                make_task(2, 0, 4, 10.0, 2, "compute"),
                make_task(3, 4, 8, 10.0, 2, "io"),
                make_task(4, 4, 8, 10.0, 2, "io"),
                make_task(5, 4, 8, 10.0, 2, "io"),
            ],
        },
    }
...
def edge_cases() -> Dict[str, Dict[str, Any]]:
    return {
        ...
        "edge_all_overlapping.json": {
            ...
            "category_limit": 1,
            "tasks": [
                make_task(0, 0, 6, 12.0, 2, "compute"),
                make_task(1, 0, 6, 9.0, 2, "compute"),
                make_task(2, 0, 6, 11.0, 3, "io"),
                make_task(3, 0, 6, 7.0, 2, "network"),
            ],
        },
        "edge_capacity_one.json": {
            ...
            "category_limit": 2,
            "tasks": [
                make_task(0, 0, 2, 4.0, 1, "compute"),
                make_task(1, 1, 3, 6.0, 1, "io"),
                make_task(2, 3, 5, 5.0, 1, "compute"),
                make_task(3, 4, 6, 3.0, 1, "network"),
            ],
        },
    }
